    ...
    tools: [
      {
        ...
        description:
          "A tool to search technical documentation on MDN (Mozilla Developer Network).",
        inputSchema: {
          type: "object",
          properties: {
            searchTerm: {
              type: "string",
              description:
                "The search term to look for in the MDN documentation",
            },
          },
          required: ["searchTerm"],
        },
      },
      {
        ...
        description:
          "Tool to get common code snippets and templates for various programming languages",
        inputSchema: {
          type: "object",
          properties: {
            language: {
  ...
  if (request.params.name === "search-mdn-docs") {
    ...
    try {
      const response = await fetch(
        `https://developer.mozilla.org/api/v1/search?q=${encodeURIComponent(searchTerm)}&locale=en-US`,
      );

      if (!response.ok) {
        throw new Error(
          `Failed to fetch MDN docs for search term: ${searchTerm}`,
        );
      }
      ...
      const results = data.documents.slice(0, 10).map((doc: any) => {
        return `### ${doc.title}\n**URL:** ${doc.mdn_url}\n**Summary:** ${doc.summary}\n`;
      }).join('\n---\n');
      ...
      return {
        content: [
          {
            type: "text",
            text: `Error searching MDN docs: ${(error as Error).message}`,
          },
        ],
        isError: true,
      };
