        ...
        inputSchema: {
          ...
          properties: {
            city: {
              type: "string",
              description:
                "The name of the city for which you want to know the current weather",
            },
          },
          required: ["city"],
        },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "get-city-weather") {
    ...
    // Validation côté serveur : vérifier que city est bien fournie
    if (!city || city === "undefined" || city.trim() === "") {
      return {
        content: [
          {
            type: "text",
            text: `Error: The 'city' parameter is required.`,
          },
        ],
        isError: true,
      };
