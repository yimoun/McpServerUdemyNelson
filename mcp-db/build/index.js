...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "get-categories",
                description: "A tool to get a list of categories of the blog from the database",
                inputSchema: {
                    type: "object",
                    properties: {},
                },
            },
            {
                ...
                description: "A tool to get articles by category name from the database",
                inputSchema: {
                    type: "object",
                    properties: {
                        categoryName: {
            ...
            },
            {
                ...
                description: "Tool to fetch recent error logs from the server for debugging purposes",
                inputSchema: {
                    type: "object",
                    properties: {},
                },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (request.params.name === "get-articles-by-category") {
        ...
        try {
            const response = await fetch(`http://localhost:3000/categories/nom/${encodeURIComponent(categoryName)}/articles`);
            const data = await response.json();
            if (data.message === "success") {
                return {
                    content: [
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Error fetching recent errors: ${error.message}`,
                    },
                ],
                isError: true,
            };
