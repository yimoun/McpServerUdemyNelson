...
class UtilityMCPServer {
      ...
      tools: [
        {
          ...
          description:
            "Génère un mot de passe sécurisé avec différentes options (longueur, caractères spéciaux, chiffres, majuscules)",
          inputSchema: {
            type: "object",
            properties: {
              length: {
        ...
        },
        {
          name: "validate-email",
          description:
            "Valide une adresse email et retourne si elle est correcte ou non",
          inputSchema: {
            type: "object",
            properties: {
              email: {
  ...
  private validateEmail(args: any) {
    const { email } = args;
  ...
  private shortenUrl(args: any) {
    ...
    if (customCode) {
      if (customCode.length < 4 || customCode.length > 10) {
        throw new Error(
          "Le code personnalisé doit faire entre 4 et 10 caractères",
        );
      }
  ...
  private getSystemInfo(args: any) {
